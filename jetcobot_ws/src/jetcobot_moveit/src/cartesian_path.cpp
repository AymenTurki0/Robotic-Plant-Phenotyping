...
public:
  ...
  void initialize()
  {
    ...
    initial_pose.position.x = 0.12851;
    initial_pose.position.y = -0.017044;
    initial_pose.position.z = 0.36093;
    initial_pose.orientation.x = -6.6973;
    initial_pose.orientation.y = 3.4232;
    initial_pose.orientation.z = -1.6603;
    ...
    if (success)
    {
      ...
      target_pose.position.x = 0.36093;
      waypoints.push_back(target_pose);
      target_pose.position.y = -0.022044;
      target_pose.position.x =  0.15851;
      target_pose.position.x = 0.25093;
      waypoints.push_back(target_pose);
      target_pose.position.x = 3.1805826438358054e-05;
      target_pose.position.y = -4.311262455303222e-05;
      target_pose.position.z = -3.6135115806246176e-05;
      waypoints.push_back(target_pose);
      ...
      if (fraction >= 0.0)
      {
        ...
        // 在RViz中可视化轨迹
        visual_tools.publishTrajectoryLine(trajectory_msg, move_group_interface_->getRobotModel()->getLinkModel("jiazhua_Link"), joint_model_group, rviz_visual_tools::LIME_GREEN);
