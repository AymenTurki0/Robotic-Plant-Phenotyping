...
public:
  ...
  void initialize()
  {
	...
	{
		attempt_count++;
		std::vector<double> target_joints = {0.0, -1.5454, 0.0, 1.5454, 0.0,0.0}; // 目标关节角度（单位：弧度）
		// 设置目标关节空间值
    	...
    	{
          // 在RViz中可视化轨迹
      	visual_tools_.publishTrajectoryLine(my_plan.trajectory,move_group_interface_->getRobotModel()->getLinkModel("jiazhua_Link"),joint_model_group,rviz_visual_tools::LIME_GREEN);
